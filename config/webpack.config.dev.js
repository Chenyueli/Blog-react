...
module.exports = {
  ...
  module: {
    ...
    rules: [
      ...
      // before Babel processes the JS.
      {
        ...
        //“oneOf”将遍历所有后续加载器，直到符合要求。 当没有加载器匹配时，它将回退到加载器列表末尾的“文件”加载器。
        oneOf: [
          ...
          // “postcss”加载器将autoprefixer应用于我们的CSS。 “css”加载器解析CSS中的路径并将资源添加为依赖项。 
          // “style”加载器将CSS转换为注入<style>标签的JS模块。
          // 在生产中，我们使用插件将CSS提取到文件中，但在开发中，“样式”加载器可以对CSS进行热编辑。
  ...
  plugins: [
    ...
    //生成一个带有<script>的`index.html`文件。
    new HtmlWebpackPlugin({ inject: true, template: paths.appHtml }),
    //将模块名称添加到工厂函数，以便它们出现在浏览器分析器中。
